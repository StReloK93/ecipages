<template>
   <div class="mb-4 h-full" v-if="props.transport.smena && props.transport.start_smena_day">
      <nav class="mb-6">
         <DatePicker v-model="date" @update:model-value="selectMonth" view="month" dateFormat="yy-mm" size="small" />
      </nav>
      <table>
         <tbody>
            <tr>
      ...
      if (differentDay % smenaDays == 0) {
         ...
         if (day <= smenaDays) {
            ...
            for (let index = smenaDays - 1; index > 0; index--) {
               ...
               if (currentDay.getMonth() == nextDay.getMonth()) startArray.unshift({ date: nextDay, number: index + 1 })
